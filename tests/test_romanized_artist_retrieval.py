...
def test_retrieval_artist_name_reverses_musicbrainz_sort_name(monkeypatch):
    monkeypatch.setattr(spotify, "_musicbrainz_artist_ids", lambda _name: {"artist-mbid"})
    monkeypatch.setattr(
        spotify,
        "_musicbrainz_get",
        lambda path, _params: {
            "id": "artist-mbid", "name": "山下達郎",
            "sort-name": "Yamashita, Tatsuro", "aliases": [],
        } if path == "artist/artist-mbid" else None,
    )
...
def test_retrieval_artist_name_prefers_explicit_latin_alias(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        "_musicbrainz_get",
        lambda path, _params: {
            "id": "artist-mbid", "name": "山根麻以",
            "sort-name": "Yamane, Mai", "aliases": [{"name": "Mai Yamane"}],
        } if path == "artist/artist-mbid" else None,
    )
...
def test_only_second_spotify_search_uses_romanized_artist(monkeypatch):
    ...
    def fake_mb_get(path, _params):
        if path == "artist/artist-mbid":
            return {"id": "artist-mbid", "name": "来生たかお", "sort-name": "Kisugi, Takao", "aliases": []}
    ...
    monkeypatch.setattr(spotify, "_musicbrainz_recording_identity_accepts", lambda *_args, **_kwargs: True)
    queries = []
    ...
    result = spotify.search_track("token", "余韻", ["来生たかお"], "Album", duration_ms=240000)
    assert result == "match"
...
def test_first_query_match_does_not_resolve_romanized_artist(monkeypatch):
    ...
    monkeypatch.setattr(spotify, "_spotify_get", fake_spotify_get)
    result = spotify.search_track("token", "Sparkle", ["山下達郎"], "Album", duration_ms=240000)
    assert result == "match"
...
def test_first_search_candidates_prevent_romanized_second_query(monkeypatch):
    mb_artist_lookup_called = False
    def fail_if_called(_name):
        nonlocal mb_artist_lookup_called
        mb_artist_lookup_called = True
        return {"artist-mbid"}
    monkeypatch.setattr(spotify, "_musicbrainz_artist_ids", fail_if_called)
    wrong = _candidate(track_id="wrong", title="Different Song", artist="山下達郎")
    queries = []
    responses = [FakeResponse([wrong]), FakeResponse([])]
    def fake_spotify_get(url, _token, params):
        assert url.endswith("/search")
        queries.append(params["q"])
        return responses.pop(0)
    ...
    assert len(queries) == 2
    assert all('artist:"山下達郎"' in query for query in queries)
    assert mb_artist_lookup_called is False
